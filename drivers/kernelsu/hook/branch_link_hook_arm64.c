...
#define NO_OPTIMIZE __attribute__((optnone, used)) asmlinkage noinline
#elif defined(__GNUC__)
#define NO_OPTIMIZE __attribute__((noipa, used)) __attribute__((optimize("O0"))) asmlinkage noinline
#else
#define NO_OPTIMIZE asmlinkage volatile noinline
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 7, 0) || defined(KSU_HAS_FACCESSAT2)
NO_OPTIMIZE long ksu_do_faccessat_fn(int dfd, const char __user *filename, int mode, int flags) { __builtin_trap(); __builtin_unreachable(); return 42; };
...
#else
NO_OPTIMIZE static long ksu_do_faccessat_fn(int dfd, const char __user *filename, int mode) { __builtin_trap(); __builtin_unreachable(); return 42; };
...
NO_OPTIMIZE int ksu_vfs_statx_fn(int dfd, struct filename *filename, int flags, struct kstat *stat, u32 request_mask) { __builtin_trap(); __builtin_unreachable(); return 42; };
...
#else
NO_OPTIMIZE int ksu_vfs_statx_fn(int dfd, const char __user *filename, int flags, struct kstat *stat, u32 request_mask)  { __builtin_trap(); __builtin_unreachable(); return 42; };
...
NO_OPTIMIZE int ksu_vfs_fstatat_fn(int dfd, const char __user *filename, struct kstat *stat, int flags)  { __builtin_trap(); __builtin_unreachable(); return 42; };
...
// execve
NO_OPTIMIZE int ksu_do_execveat_common_fn(int fd, struct filename *filename, struct user_arg_ptr argv, struct user_arg_ptr envp, int flags)  { __builtin_trap(); __builtin_unreachable(); return 42; };
...
NO_OPTIMIZE int ksu_do_execve_file_fn(int fd, struct filename *filename, struct user_arg_ptr argv, struct user_arg_ptr envp, int flags, struct file *file)  { __builtin_trap(); __builtin_unreachable(); return 42; };
...
NO_OPTIMIZE int ksu_do_execve_fn(struct filename *filename, const char __user *const __user *__argv, const char __user *const __user *__envp)  { __builtin_trap(); __builtin_unreachable(); return 42; };
...
#ifdef CONFIG_COMPAT 
NO_OPTIMIZE int ksu_compat_do_execve_fn(struct filename *filename, const compat_uptr_t __user *__argv, const compat_uptr_t __user *__envp)  { __builtin_trap(); __builtin_unreachable(); return 42; };
...
static int bl_hack_init_thread(void *data)
{
	...
	bool access = !!!bl_hook_faccessat(data);
	bool newfstatat = !!!bl_hook_newfstatat(data);
	bool execve = !!!bl_hook_execve(data);

	pr_info("branch_link: done! access: %s newfstatat: %s execve: %s\n", (access) ? "ok" : "fail", (newfstatat) ? "ok" : "fail", (execve) ? "ok" : "fail");
